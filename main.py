...
async def on_message(message):
    ...
    if "won the" in message.content.lower() and "Congratulations" in message.content:
        ...
        try:
            ...
            # Récupérer les entrées du giveaway précédent de manière plus fiable
            entries = []
            async for msg in message.channel.history(limit=10):
                if msg.author.id == TARGET_BOT_ID and "entries" in msg.content.lower():
                    entries = [entry.strip() for entry in msg.content.split("\n") if "." in entry and "(" in entry]
                    break

            if entries and winner:
                # Filtrer les entrées pour exclure le gagnant
                valid_entries = [entry for entry in entries if winner.lower() not in entry.lower()]
                
                if valid_entries:
                    loser_entry = random.choice(valid_entries)
                    try:
                        ...
                        try:
                            ...
                            await message.channel.send("GG au gagnant ! 🎉")
                    except Exception as e:
                        print(f"❌ Erreur lors du traitement du perdant: {e}")
                else:
                    print("❌ Aucun perdant valide trouvé")
            else:
                print("❌ Aucune entrée ou gagnant valide trouvé")
            ...
            for croupier_id, croupier_data in all_commissions.items():
                ...
                if channel_id:
                    ...
                    if channel:
                        ...
                        # Calculer la répartition des commissions
                        for server, server_data in croupier_data["servers"].items():
                            commission = float(server_data["commission"])
                            ...
                            embed.add_field(
                                ...
                                value=f"Commission totale: **{commission:.2f}**\n\n" +
                                      f"👑 **50%** VIP: **{vip_part:.2f}**\n" +
                                      f"💼 **10%** Investissement: **{invest_part:.2f}**\n" +
                                      f"🎲 **40%** Croupier: **{croupier_part:.2f}**",
                                inline=False
                            )
        ...
        if status["current_vip"] is not None:
            embed.add_field(
                ...
                value=f"Niveau VIP actuel : **{status['current_vip']}**\n" +
                      ...
                      (f"Reste à miser : **{status['remaining']}**" if status['next_threshold'] else "🎉 Niveau maximum atteint !"),
                inline=False
            )
